...
let datap = [];
let m = new Date();
let s = ['January','February','March','April','May','June','July',
...
function updateTop(){
    ...
    datap = Array.from(m).map(e => parseFloat(e.innerText));
    datam = Array.from(p).map(e => parseFloat(e.innerText));
    let sumP = +datap.reduce(((a,b) => a + b), 0).toFixed(2);
    let sumM = -datam.reduce(((a,b) => a + b), 0).toFixed(2);
